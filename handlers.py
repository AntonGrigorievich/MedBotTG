...
async def answer_keyworrds(message: Message):
    await message.answer('Задайте ваш вопрос в текстовом формате')
    await KeywordsAnswer.first()

@dp.message_handler(state=KeywordsAnswer.get_question)
async def send_keywords_answer(message: Message, state: FSMContext):
    text = find_keywords(message.text)
